...
//            }
        ...
        for (row in rows) {
            ...
            if (queryInstance.save(flush:true)) {
                ...
                if (row[12]) {
                    ...
                    fileUploadInstance.filePath = File.separator + 'var' + File.separator + 'storage' + File.separator + 'cdr-filestore' + File.separator + Organization.findByCode('DCC').code + File.separator + 'query_tracker' + File.separator + queryInstance.id
                    if (row[11] == 'DCF') {
    ...
    def createDir = {
        def dirPath = File.separator + 'var' + File.separator + 'storage' + File.separator + 'cdr-filestore' + File.separator + Organization.findByCode('DCC').code + File.separator + 'query_tracker'
    ...
    def migrateFiles = {
        ...
        Query.list().each { queryInstance ->
            queryInstance.queryAttachments.each {
                filePath = File.separator + 'var' + File.separator + 'storage' + File.separator + 'cdr-filestore' + File.separator + Organization.findByCode('DCC').code + File.separator + 'query_tracker'
    ...
    Map checkError(queryInstance) {
        ...
        if (queryInstance.organization && queryInstance.caseRecord) {
            if (queryInstance.organization?.code == 'BROAD') {
                ...
                if (queryInstance.organization?.code?.split('-')[0] != queryInstance.caseRecord?.bss?.code) {
        ...
        if (queryInstance.organization && queryInstance.candidateRecord) {
            if (queryInstance.organization?.code == 'VARI' || queryInstance.organization?.code == 'BROAD' || queryInstance.organization?.code == 'MBB') {
                ...
                if (queryInstance.organization?.code?.split('-')[0] != queryInstance.candidateRecord?.bss?.code) {
