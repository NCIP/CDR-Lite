  <div id="HRTTableContent">
    <table class="tabledatecomments">
      ...
      <g:each in="${clinicalDataEntryInstance.therapyRecords}" status="i" var="therapyRecord">
        <g:if test="${therapyRecord.typeOfTherapy == 'HRT'}">
          ...
          <g:else>
              ...
              <tr id="hrtAddNew" class="hide">
                <td class="textcenter"><g:select name="formHorReplaceTher" from="${nci.bbrb.cdr.forms.TherapyRecord$HRTForm?.values()}" keys="${nci.bbrb.cdr.forms.TherapyRecord$HRTForm?.values()*.name()}" /></td>
                <td class="textcenter"><g:textField size="30" maxlength="255" name="formHRTOtherSpecification"/></td>
                <td class="textcenter"><g:select name="typeHorReplaceTher" from="${nci.bbrb.cdr.forms.TherapyRecord$HRTType?.values()}" keys="${nci.bbrb.cdr.forms.TherapyRecord$HRTType?.values()*.name()}" /></td>
