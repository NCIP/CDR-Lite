...
<html>
    ...
    <body>
        ...
        <div id="container" class="clearfix">
            <h1><g:message code="default.edit.label.with.case.id" args="['Slide Sectioning Form',caseId]" /></h1>
            <g:if test="${flash.message}">
                <div class="message">${flash.message}</div>
            </g:if>
            <g:hasErrors bean="${slideSectionInstance}">
                <div class="errors">
                    <g:renderErrors bean="${slideSectionInstance}" as="list" />
                </div>
            </g:hasErrors>
            <g:queryDesc caserecord="${slideSectionInstance?.caseRecord}" form="bpvStain" />
            <g:form method="post" action="update">
                <g:hiddenField name="id" value="${slideSectionInstance?.id}"/>
                <g:hiddenField name="version" value="${slideSectionInstance?.version}"/>
